...
export default function GstRegistrationsSection({ companyId }: Props) {
  ...
  const renderRow = (r: GstRegistrationDto) => {
    ...
    return (
      <div key={r.id} className="flex items-center gap-3 px-4 py-3 bg-gray-900/50 border border-gray-800 rounded-lg">
        {isEditing ? (
          <input
            type="text"
            value={editing!.gstin}
            ...
            maxLength={15}
            className="flex-1 px-3 py-2 bg-gray-900 border border-gray-700 rounded text-white text-sm font-mono uppercase focus:outline-none focus:border-gray-500"
            autoFocus
          />
        ) : (
          <span className="flex-1 font-mono text-sm text-white tracking-wider">{r.gstin}</span>
        )}
        <span className="w-40 text-sm text-gray-300 truncate">{r.state || '—'}</span>
        {klass && (
          ...
          {isEditing ? (
            <>
              <button
                ...
                className="px-3 py-1 text-xs bg-white text-black rounded hover:bg-gray-200 disabled:opacity-50"
              >
                Save
              </button>
              <button
            ...
            <>
              ...
              <button
                ...
                disabled={busyId === r.id}
                className="px-2 py-1 text-xs text-gray-500 hover:text-red-400 transition-colors disabled:opacity-50"
  ...
  return (
    <div className="bg-primary-dark-card border border-gray-800 rounded-2xl p-6 space-y-5">
      ...
      <div className="flex items-center gap-3">
        <input
          ...
          maxLength={15}
          className="flex-1 px-3 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white text-sm font-mono uppercase focus:outline-none focus:border-gray-500"
          onKeyDown={(e) => { if (e.key === 'Enter') handleAdd() }}
        />
        <div className="w-40 px-3 py-2 text-xs text-gray-400 truncate">
        ...
        <button
          ...
          className="px-4 py-2 bg-white text-black rounded-lg text-sm font-medium hover:bg-gray-200 transition-colors disabled:opacity-50"
        >
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {within.length > 0 && (
            <div>
              <h4 className="text-xs uppercase tracking-wider text-gray-500 mb-2">Within State</h4>
              <div className="space-y-2">{within.map(renderRow)}</div>
            </div>
          )}
          {outside.length > 0 && (
            <div>
              <h4 className="text-xs uppercase tracking-wider text-gray-500 mb-2">Outside State</h4>
              <div className="space-y-2">{outside.map(renderRow)}</div>
            </div>
          )}
          {unclassified.length > 0 && (
            <div>
              <h4 className="text-xs uppercase tracking-wider text-gray-500 mb-2">Unclassified</h4>
