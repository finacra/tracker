  ...
  return (
    <div className="bg-gray-900/50 border border-gray-700/50 rounded-lg p-4">
      <h4 className="text-sm font-medium text-white mb-3">
        Advance Tax Calculator {financialYear && <span className="text-gray-400 font-normal">— {financialYear}</span>}
      </h4>

      {/* Estimated Liability Input */}
      <div className="mb-4">
        <label className="block text-xs text-gray-400 mb-1">Estimated Tax Liability (₹)</label>
        <input
          type="number"
          value={estimatedLiability}
          onChange={(e) => setEstimatedLiability(e.target.value)}
          placeholder="e.g. 1000000"
          className="w-full sm:w-64 px-3 py-1.5 bg-gray-800 border border-gray-600 rounded text-white text-sm focus:outline-none focus:border-gray-500"
        />
      </div>
            ...
            <table className="w-full text-xs sm:text-sm">
              ...
              <tbody>
                {summary.installments.map((inst) => (
                  <tr key={inst.quarter} className="border-b border-gray-800">
                    <td className="py-2 pr-3 text-white font-medium">
                      {inst.quarter} ({inst.cumulativePercent}%)
                    </td>
                    <td className="py-2 pr-3 text-gray-300">{inst.dueDate}</td>
                    <td className="py-2 pr-3 text-right text-gray-300">{formatINR(inst.estimatedAmount)}</td>
                    <td className="py-2 pr-3 text-right">
                      <input
                        ...
                        placeholder="0"
                        className="w-24 px-2 py-1 bg-gray-800 border border-gray-600 rounded text-white text-right text-xs focus:outline-none focus:border-gray-500"
                      />
                    </td>
                    <td className={`py-2 pr-3 text-right ${inst.shortfall > 0 ? 'text-red-400' : 'text-green-400'}`}>
                      {inst.shortfall > 0 ? formatINR(inst.shortfall) : '—'}
                    </td>
                    <td className={`py-2 text-right ${inst.interestAmount > 0 ? 'text-red-400' : 'text-gray-500'}`}>
              ...
              <tfoot>
                <tr className="border-t border-gray-600">
                  ...
                  <td className={`py-2 text-right font-medium ${summary.totalInterest > 0 ? 'text-red-400' : 'text-gray-500'}`}>
