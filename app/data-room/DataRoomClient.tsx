...
function DataRoomPageInner({
  ...
  initialInitResult: Awaited<ReturnType<typeof getDataRoomInitState>> | null
}) {
  ...
  useEffect(() => {
    ...
    async function initializeDataRoom() {
      ...
      try {
        ...
        // PR-44: when the RSC page wrapper supplied a server-prefetched
        // init result, consume it directly. We skip the client-side
        // server-action round-trip (which previously cost ~2.5-4s of
        // visible loading on every cold page load) entirely on the
        // first mount. Subsequent navigations or full reloads where
        // the prop isn't present (e.g. dev HMR boundary hits) fall
        // through to the original fetch path unchanged.
        //
        // Bypass condition: prop is present AND it was prefetched for
        // the same company the URL currently asks for. If the URL has
        // company_id=X but the prefetched result is for Y (user
        // changed URL after initial render — unlikely on first mount
        // but cheap to guard), we re-fetch to honor the URL.
        const propMatchesUrl =
          !!initialInitResult &&
          initialInitResult.success &&
          (!initialCompanyId ||
            initialInitResult.data?.currentCompanyId === initialCompanyId ||
            initialInitResult.data?.accessibleCompanyIds?.includes(initialCompanyId))
        const result = propMatchesUrl
          ? initialInitResult!
          : await getDataRoomInitState(initialCompanyId);
...
/**
 * Server-prefetched init payload. The RSC page wrapper
 * (app/data-room/page.tsx) calls getDataRoomInitState during render
 * and passes the result down so the client useEffect can consume it
 * synchronously on mount — skipping the ~2.5-4s client-side server-
 * action round-trip that previously gated every page load.
 *
 * Old contract preserved: when the prop is absent (e.g. dev-time HMR
 * navigation that hits the client boundary directly), the existing
 * client-side fetch path still runs unchanged.
 */
export type InitialInitResult = Awaited<ReturnType<typeof getDataRoomInitState>>

export default function DataRoomClient({
  initialInitResult,
}: {
  initialInitResult?: InitialInitResult | null
}) {
  return (
    <Suspense
      fallback={
    ...
    >
      <DataRoomPageInner initialInitResult={initialInitResult ?? null} />
    </Suspense>
  );
}
