...
export default function BulkUploadPage() {
  ...
  useEffect(() => {
    ...
    const initHandsontable = async () => {
      ...
      // ============================================
      // Custom multi-select checkbox editor
      // (writes comma-separated text back into the cell)
      // ============================================
      ...
      class MultiCheckboxEditor extends (Handsontable as any).editors.BaseEditor {
        ...
        selectAllBtn: HTMLButtonElement | null = null
        ...
        init() {
          ...
          this.container.className = 'hot-multicheckbox-editor'
          ...
          this.container.style.boxShadow = '0 8px 28px rgba(0,0,0,0.18)'
          ...
          this.selectAllBtn.textContent = 'All'
          this.selectAllBtn.style.padding = '8px 10px'
          this.selectAllBtn.style.fontSize = '12px'
          this.selectAllBtn.style.borderRadius = '8px'
          this.selectAllBtn.style.border = '1px solid #0078d4'
          this.selectAllBtn.style.background = '#e5f3ff'
          this.selectAllBtn.style.color = '#0078d4'
          this.selectAllBtn.style.cursor = 'pointer'
        ...
        open() {
          if (!this.container || !this.listEl || !this.searchInput || !this.applyBtn || !this.cancelBtn || !this.selectAllBtn || !this.clearBtn) return
          ...
          const width = Math.min(520, Math.max(360, tdRect?.width ? tdRect.width * 2 : 420))
          this.container.style.minWidth = `${width}px`

          const left = Math.min(viewportW - width - 12, Math.max(12, (tdRect?.left ?? 12)))
          const top = Math.min(viewportH - 360 - 12, Math.max(12, (tdRect?.bottom ?? 12) + 6))
          ...
          const render = () => {
            ...
            const isAllSelected = this.options.length > 0 && this.selected.size === this.options.length

            const allRow = document.createElement('label')
            allRow.style.display = 'flex'
            allRow.style.alignItems = 'center'
            allRow.style.gap = '8px'
            allRow.style.padding = '8px 10px'
            allRow.style.border = '1px solid #d4d4d4'
            allRow.style.borderRadius = '8px'
            allRow.style.background = isAllSelected ? '#e5f3ff' : '#fafafa'
            allRow.style.gridColumn = '1 / -1'
            ...
            allText.style.fontSize = '12px'
            allText.style.fontWeight = '600'

            allRow.appendChild(allCb)
            allRow.appendChild(allText)
            this.listEl!.appendChild(allRow)
            ...
            allCb.addEventListener('change', (e) => onToggleAll((e.target as HTMLInputElement).checked))
          ...
          const onSelectAllBtn = () => { this.options.forEach(o => this.selected.add(o)); render() }
          ...
          this.selectAllBtn.addEventListener('click', onSelectAllBtn)
          ...
          const onOutsidePointer = (e: MouseEvent) => {
            ...
            // Cancel on outside click to avoid accidental overwrites
            onCancel()
          }
          ...
          this.cleanupFns.push(() => this.selectAllBtn?.removeEventListener('click', onSelectAllBtn))
          ...
          // Focus search for fast selection
      ...
      const columns = CSV_COLUMNS.map((col, idx) => {
        ...
        if (dropdownColumns.includes(col)) {
          ...
          // Custom checkbox editor (writes comma-separated list)
          base.type = 'text'
